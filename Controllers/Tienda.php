	...
	class Tienda extends Controllers{
		...
		public function procesarVenta(){
			if($_POST){
				...
				$direccionenvio = strClean($_POST['direccion']).', '.strClean($_POST['ciudad']).', <br> Nombre Comprador:'.strClean($_POST['nombre']).','.strClean($_POST['apellido']).' <br> Dni Comprador:'.strClean($_POST['dni']);
				$status = "Pendiente";
				$subtotal = 0;
				...
				$password = $pass;
				$tipoid = "3";
				$dirFiscal = strClean($_POST['domicilio']).', '.strClean($_POST['provincia']);
				...
				if(!empty($_SESSION['arrCarrito'])){
					...
					if(empty($_POST['datapay'])){
						//Crear pedido
						$request_cliente = $this->insertCliente($identificacion,
															$nombre, 
															$apellido, 
															$telefono,
															$email,
		...
		public function confirmarpedido(){
			if(empty($_SESSION['dataorden'])){
				...
				$data['orden'] = $idpedido;
				$data['tipopago'] = $dataorden['listpago'];
				$data['transaccion'] = $transaccion;
				$data['mobbex'] = $dataorden['mobbex'];
			...
			if ($_POST) {
				...
				try {
					...
					if ($userContact > 0) {
						...
						// Para depuración: registrar la respuesta exitosa
						error_log("Respuesta exitosa: " . print_r($arrResponse, true));
						...
						// Para depuración: registrar la respuesta fallida
						error_log("Respuesta fallida: " . print_r($arrResponse, true));
					}
				} catch (Exception $e) {
					// Registrar el error
					error_log("Error en el proceso: " . $e->getMessage());
					$arrResponse = array('status' => false, 'msg' => "Error en el servidor. Intente nuevamente más tarde.");
				}
		
				echo json_encode($arrResponse, JSON_UNESCAPED_UNICODE);
			} else {
				// Para depuración: registrar el caso en que no hay POST
				error_log("No se recibieron datos POST.");
