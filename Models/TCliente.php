...
trait TCliente{
	...
	public function insertCliente(string $identificacion, string $nombre, string $apellido, int $telefono, string $email, string $password, int $tipoid, string $dirFiscal){
		$this->con = new Mysql();
		...
		if(empty($request))
		{
			$query_insert  = "INSERT INTO persona(identificacion,nombres,apellidos,telefono,email_user,password,rolid,direccionfiscal) 
							  VALUES(?,?,?,?,?,?,?,?)";
	...
	public function setContacto(string $nombre, string $email, string $telefono, string $mensaje, string $ip, string $dispositivo, string $useragent){
		try {
			...
			$query_insert = "INSERT INTO contacto(nombre, email, telefono, mensaje, ip, dispositivo, useragent) VALUES(?, ?, ?, ?, ?, ?, ?)";
			$arrData = array($nombre, $email, $telefono, $mensaje, $ip, $dispositivo, $useragent);
			
			// Para depuración: registrar los datos a insertar
			error_log("Datos a insertar: " . print_r($arrData, true));
			
			$request_insert = $this->con->insert($query_insert, $arrData);
			
			// Para depuración: registrar la respuesta de la inserción
			error_log("Respuesta de inserción: " . print_r($request_insert, true));
